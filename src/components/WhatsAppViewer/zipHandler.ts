...
export async function loadSampleChat(onProgress?: ProgressCallback): Promise<ChatSession> {
  ...
  const sampleUrl = '/WhatsApp Chat with Yaseen Shehab.zip';
  
  const response = await fetch(sampleUrl);
  if (!response.ok) {
    throw new Error(`Failed to load sample chat archive (Status: ${response.status} ${response.statusText})`);
  }

  const blob = await response.blob();
  return processWhatsAppFile(blob, 'WhatsApp Chat with Yaseen Shehab.zip', onProgress);
}
