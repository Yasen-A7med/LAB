...
const detectMobileDevice = (): boolean => {
  ...
  // Vector 1: User Agent string matching mobile OS/browsers
  ...
  // Vector 2: Client Hints User Agent Data API
  const isUaDataMobile = (navigator as any).userAgentData?.mobile === true;

  // Vector 3: CSS Screen boundary check (under 768px width)
  const isSmallScreen = window.innerWidth < 768;

  // Vector 4: Touch capability without fine mouse pointer
  const hasCoarseTouchOnly = 
    ('ontouchstart' in window || navigator.maxTouchPoints > 0) &&
    window.matchMedia('(pointer: coarse)').matches;

  // Vector 5: Touch screen with portrait/mobile aspect ratio
...
export const YDApp: React.FC<YDAppProps> = ({ onBack }) => {
  // Mobile device restriction check
